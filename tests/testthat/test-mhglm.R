...
test_that("succeeds on sleepstudy with uncorrelated covariates assumption", {
    model <- mhglm(Reaction ~ Days + (Days || Subject), data=sleepstudy)
    ...
    # vcov
    vcov0 <- matrix(c(47.0, -2, -2.0, 2.4), 2, 2)
    rownames(vcov0) <- colnames(vcov0) <- c("(Intercept)", "Days")
    expect_that(round(vcov(model), 1), equals(vcov0))

    # VarCorr
    varcor0 <- matrix(c(583.5, 0.0, 0.0, 33.6), 2, 2)
    rownames(varcor0) <- colnames(varcor0) <- c("(Intercept)", "Days")
    varcor <- VarCorr(model)[["Subject"]]
    expect_that(attr(varcor, "stddev"), equals(sqrt(diag(varcor))))
    expect_that(as.vector(attr(varcor, "correlation")),
    ...
    # ranef
    ranef0 <- matrix(c( 1.9, -40.0, -38.7, 23.9, 22.4, 9.0, 16.8, -7.3, 0.1, 
		       34.5, -25.2, -13.3, 4.7, 20.7, 3.2, -25.8, 0.8, 12.2,
		       9.2, -8.6, -5.4, -4.9, -3.1, -0.3, -0.2, 1.1, -10.8, 
		       8.6, 1.2, 6.6, -3.0, 3.5, 0.9, 4.9, -1.0, 1.3),
                     18, 2)
    rownames(ranef0) <- as.character(c(308, 309, 310, 330, 331, 332, 333, 334,
                                       335, 337, 349, 350, 351, 352, 369, 370,
                                       371, 372))
...
test_that("succeeds on sleepstudy in parallel with uncorrelated covariates assumption", {
